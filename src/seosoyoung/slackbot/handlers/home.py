...
def build_home_view(
    ...
    active_sessions = [
        s for s in sessions
        if s.get("status") in ("running", "error", "interrupted")
    ]
    completed_sessions = [
        s for s in sessions
    ...
    # 완료 세션: updated_at 기준 최신순 정렬, 최대 5개
    completed_sessions.sort(
        key=lambda s: s.get("updated_at", ""),
        reverse=True,
    )
    completed_sessions = completed_sessions[:MAX_COMPLETED_SESSIONS]
    ...
    blocks.append({
        ...
        "elements": [{
            "type": "mrkdwn",
            "text": f"최근 완료 세션 {len(completed_sessions)}개 표시 중  ·  탭을 다시 열면 갱신됩니다",
        }],
    })
