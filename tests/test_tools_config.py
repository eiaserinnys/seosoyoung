...
class TestRoleToolsSingleSource:
    ...
    def test_admin_role_exists(self):
        """admin 역할이 존재"""
        assert "admin" in Config.ROLE_TOOLS

    def test_viewer_role_exists(self):
        """viewer 역할이 존재"""
        assert "viewer" in Config.ROLE_TOOLS
    ...
    def test_agent_runner_uses_config(self):
        """agent_runner가 Config.ROLE_TOOLS를 참조하는지 확인"""
        from seosoyoung.claude.agent_runner import ClaudeRunner

        runner = ClaudeRunner()
        # 기본 allowed_tools가 Config.ROLE_TOOLS["admin"]과 동일
        assert runner.allowed_tools == Config.ROLE_TOOLS["admin"]
    ...
    def test_no_duplicate_default_allowed_tools(self):
        ...
        assert not hasattr(ar, "DEFAULT_ALLOWED_TOOLS"), \
            "DEFAULT_ALLOWED_TOOLS should be removed; use Config.ROLE_TOOLS instead"
