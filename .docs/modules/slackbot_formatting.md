...
### `format_progress_placeholder(text)`
- 위치: 줄 142
- 설명: 진행 상태로 갱신된 플레이스홀더 포맷
...
### `build_input_request_blocks(request_id, questions, agent_session_id)`
- 위치: 줄 278
- 설명: AskUserQuestion 이벤트를 Slack Block Kit으로 변환
...
### `format_input_request_answered(questions, answers)`
- 위치: 줄 355
- 설명: 응답 완료된 AskUserQuestion을 텍스트로 변환
