# channel_observer/scheduler.py

> 경로: `seosoyoung/slackbot/plugins/channel_observer/scheduler.py`
...
- `__init__(self)` (줄 26): 
- `start(self)` (줄 63): 스케줄러를 시작합니다.
- `stop(self)` (줄 74): 스케줄러를 중지합니다.
- `_schedule_next(self)` (줄 82): 다음 실행을 예약합니다.
- `_tick(self)` (줄 90): 주기적 실행: 각 채널의 버퍼를 체크하고 소화를 트리거합니다.
- `_check_and_digest(self)` (줄 99): 모든 관찰 채널의 pending 버퍼를 체크하여 파이프라인을 트리거합니다.
- `_run_pipeline(self, channel_id)` (줄 121): 소화/판단 파이프라인을 실행합니다.
