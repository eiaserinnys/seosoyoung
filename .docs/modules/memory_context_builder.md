...
### `ContextBuilder`
- 위치: 줄 118
- 설명: 장기 기억 + 세션 관찰 로그를 시스템 프롬프트로 변환

#### 메서드

- `__init__(self, store)` (줄 121): 
- `build_memory_prompt(self, thread_ts, max_tokens, include_persistent, include_session)` (줄 125): 장기 기억과 세션 관찰 로그를 합쳐서 시스템 프롬프트로 변환합니다.
