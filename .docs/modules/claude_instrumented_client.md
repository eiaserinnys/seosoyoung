...
- `__init__(self)` (줄 53): 
- `async receive_messages(self)` (줄 64): receive_messages를 오버라이드하여 raw 스트림에서 이벤트를 관찰.
- `_handle_rate_limit(self, data)` (줄 89): rate_limit_event 관찰.
- `_handle_unknown_event(self, msg_type, data)` (줄 97): unknown event 관찰.
