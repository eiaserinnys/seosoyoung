...
- `async execute(self, prompt, request_id, resume_session_id, on_progress, on_compact, on_debug)` (줄 38): Claude Code를 soul 서버에서 실행하고 ClaudeResult로 반환
- `async intervene(self, request_id, text, user)` (줄 133): 실행 중인 태스크에 인터벤션 전송
- `async close(self)` (줄 160): 클라이언트 종료
