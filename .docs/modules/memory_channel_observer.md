...
- `__init__(self, api_key, model)` (줄 268): 
- `async observe(self, channel_id, existing_digest, channel_messages, thread_buffers)` (줄 272): 채널 버퍼를 분석하여 관찰 결과를 반환합니다 (하위호환).
- `async digest(self, channel_id, existing_digest, judged_messages)` (줄 315): judged 메시지를 digest에 편입합니다 (소화 전용).
- `async judge(self, channel_id, digest, judged_messages, pending_messages, thread_buffers, bot_user_id, slack_client)` (줄 362): pending 메시지에 대해 리액션을 판단합니다 (판단 전용).
...
- `__init__(self, api_key, model)` (줄 418): 
- `async compress(self, digest, target_tokens)` (줄 423): digest를 압축합니다.
