...
- `_thread_buffer_path(self, channel_id, thread_ts)` (줄 143): 
- `_thread_buffer_lock(self, channel_id, thread_ts)` (줄 146): 
- `append_thread_message(self, channel_id, thread_ts, message)` (줄 149): 스레드 메시지를 버퍼에 추가
- `load_thread_buffer(self, channel_id, thread_ts)` (줄 157): 스레드 메시지 버퍼를 로드. 없으면 빈 리스트.
- `load_all_thread_buffers(self, channel_id)` (줄 167): 채널의 전체 스레드 버퍼를 로드. {thread_ts: [messages]} 형태.
- `_count_messages_tokens(self, messages)` (줄 183): 메시지 리스트의 총 토큰 수를 계산
- `count_pending_tokens(self, channel_id)` (줄 193): pending 버퍼 총 토큰 수 (채널 + 스레드 합산)
- `count_judged_plus_pending_tokens(self, channel_id)` (줄 202): judged + pending 합산 토큰 수
- `count_buffer_tokens(self, channel_id)` (줄 208): count_pending_tokens의 하위호환 별칭
- `_clear_thread_buffers(self, channel_id)` (줄 214): 스레드 버퍼 전체를 비운다.
- `clear_buffers(self, channel_id)` (줄 223): pending + judged + 스레드 버퍼를 모두 비운다.
- `_digest_path(self, channel_id)` (줄 231): 
- `_digest_meta_path(self, channel_id)` (줄 234): 
