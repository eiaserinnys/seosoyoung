...
- `snapshot(self)` (줄 161): 현재 이벤트 수 기록 (외부 루프 시작 시 호출)
- `did_compact(self, before)` (줄 165): 스냅샷 이후 compact가 발생했는지
- `can_retry(self)` (줄 169): 
- `increment(self)` (줄 172): 
...
- `has_result(self)` (줄 190): 
- `reset_for_retry(self)` (줄 193): compact retry 시 텍스트 상태 리셋
...
- `_build_options(self, session_id, compact_events, user_id, prompt)` (줄 404): ClaudeCodeOptions, OM 메모리 프롬프트, 디버그 앵커 ts, stderr 파일을 반환합니다.
- `async _notify_compact_events(self, compact_state, on_compact)` (줄 468): 미통지 compact 이벤트를 on_compact 콜백으로 전달
- `async _receive_messages(self, client, compact_state, msg_state, on_progress, on_compact)` (줄 486): 내부 메시지 수신 루프: receive_response()에서 메시지를 읽어 상태 갱신
- `_evaluate_compact_retry(self, compact_state, msg_state, before_snapshot)` (줄 624): Compact retry 판정. True이면 외부 루프 continue, False이면 break.
- `async run(self, prompt, session_id, on_progress, on_compact, user_id, user_message)` (줄 684): Claude Code 실행
- `async _execute(self, prompt, session_id, on_progress, on_compact, user_id)` (줄 704): 실제 실행 로직 (ClaudeSDKClient 기반)
- `async compact_session(self, session_id)` (줄 891): 세션 컴팩트 처리
