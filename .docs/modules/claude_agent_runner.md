...
- `__init__(self, thread_ts)` (줄 153): 
- `async shutdown_all_clients(cls)` (줄 178): 하위 호환: 모듈 레벨 shutdown_all()로 위임
- `shutdown_all_clients_sync(cls)` (줄 183): 하위 호환: 모듈 레벨 shutdown_all_sync()로 위임
- `run_sync(self, coro)` (줄 187): 동기 컨텍스트에서 코루틴을 실행하는 브릿지
- `async _get_or_create_client(self, options)` (줄 191): ClaudeSDKClient를 가져오거나 새로 생성
- `async _remove_client(self)` (줄 236): 이 러너의 ClaudeSDKClient를 정리
