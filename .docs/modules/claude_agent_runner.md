...
- `_evaluate_compact_retry(self, compact_state, msg_state, before_snapshot)` (줄 616): Compact retry 판정. True이면 외부 루프 continue, False이면 break.
- `async run(self, prompt, session_id, on_progress, on_compact)` (줄 676): Claude Code 실행
- `async _execute(self, prompt, session_id, on_progress, on_compact)` (줄 692): 실제 실행 로직 (ClaudeSDKClient 기반)
- `async compact_session(self, session_id)` (줄 862): 세션 컴팩트 처리
