...
- `snapshot(self)` (줄 159): 현재 이벤트 수 기록 (외부 루프 시작 시 호출)
- `did_compact(self, before)` (줄 163): 스냅샷 이후 compact가 발생했는지
- `can_retry(self)` (줄 167): 
- `increment(self)` (줄 170): 
...
- `has_result(self)` (줄 188): 
- `reset_for_retry(self)` (줄 191): compact retry 시 텍스트 상태 리셋
...
- `__init__(self, thread_ts)` (줄 212): 
- `async shutdown_all_clients(cls)` (줄 243): 하위 호환: 모듈 레벨 shutdown_all()로 위임
- `shutdown_all_clients_sync(cls)` (줄 248): 하위 호환: 모듈 레벨 shutdown_all_sync()로 위임
- `run_sync(self, coro)` (줄 252): 동기 컨텍스트에서 코루틴을 실행하는 브릿지
- `async _get_or_create_client(self, options)` (줄 256): ClaudeSDKClient를 가져오거나 새로 생성
- `async _remove_client(self)` (줄 301): 이 러너의 ClaudeSDKClient를 정리
