...
- `is_execution_running(self, client_id, request_id)` (줄 175): 태스크 실행이 진행 중인지 확인
- `async send_reconnect_status(self, client_id, request_id, queue)` (줄 181): 재연결 시 현재 상태 이벤트 전송
- `async cancel_running_tasks(self, timeout)` (줄 215): 실행 중인 모든 태스크 취소
