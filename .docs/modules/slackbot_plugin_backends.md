...
- `async compact(self, session_id)` (줄 448): Compact a Claude Code session.
- `get_session_id(self, thread_ts)` (줄 470): Get the Claude Code session ID for a thread.
- `is_restart_pending(self)` (줄 475): Check if a restart is pending.
- `get_data_dir(self)` (줄 479): Get the data directory for plugin storage.
...
- `__init__(self, tracker)` (줄 492): 
- `mark(self, thread_ts)` (줄 495): 
- `is_handled(self, thread_ts)` (줄 498): 
- `unmark(self, thread_ts)` (줄 501): 
