...
- `__init__(self, client)` (줄 52): Initialize with Slack WebClient.
- `async send_message(self, channel, text, thread_ts)` (줄 60): Send a message to a channel.
- `async update_message(self, channel, ts, text)` (줄 84): Update an existing message.
- `async add_reaction(self, channel, ts, emoji)` (줄 113): Add a reaction to a message.
- `async remove_reaction(self, channel, ts, emoji)` (줄 134): Remove a reaction from a message.
- `async get_user_info(self, user_id)` (줄 155): Get information about a user.
- `async get_thread_replies(self, channel, thread_ts, limit)` (줄 172): Get replies in a thread.
- `async get_channel_history(self, channel, limit)` (줄 201): Get recent messages in a channel.
- `async open_dm(self, user_id)` (줄 228): Open a DM channel with a user.
...
- `async run(self, prompt, channel, thread_ts, role, session_id, on_compact, context)` (줄 324): Execute Claude Code with the given prompt.
- `async compact(self, session_id)` (줄 454): Compact a Claude Code session.
- `get_session_id(self, thread_ts)` (줄 476): Get the Claude Code session ID for a thread.
- `is_restart_pending(self)` (줄 481): Check if a restart is pending.
- `get_data_dir(self)` (줄 485): Get the data directory for plugin storage.
...
- `__init__(self, tracker)` (줄 498): 
- `mark(self, thread_ts)` (줄 501): 
- `is_handled(self, thread_ts)` (줄 504): 
- `unmark(self, thread_ts)` (줄 507): 
