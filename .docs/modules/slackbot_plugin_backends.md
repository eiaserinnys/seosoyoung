...
- `async compact(self, session_id)` (줄 441): Compact a Claude Code session.
- `get_session_id(self, thread_ts)` (줄 463): Get the Claude Code session ID for a thread.
- `is_restart_pending(self)` (줄 468): Check if a restart is pending.
- `get_data_dir(self)` (줄 472): Get the data directory for plugin storage.
...
- `__init__(self, tracker)` (줄 485): 
- `mark(self, thread_ts)` (줄 488): 
- `is_handled(self, thread_ts)` (줄 491): 
- `unmark(self, thread_ts)` (줄 494): 
