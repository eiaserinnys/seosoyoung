...
- `_process_message(self, prompt, thread_ts, channel, user_id, say, client, is_thread_reply)` (줄 226): 공통 메시지 처리 로직 (인터벤션 지원)
- `_run_with_lock(self, prompt, thread_ts, channel, user_id, say, client, is_thread_reply)` (줄 253): 락을 보유한 상태에서 실행 (while 루프로 pending 처리)
- `_execute_once(self, prompt, thread_ts, channel, user_id, say, client, is_thread_reply)` (줄 283): 단일 Claude 실행
- `_handle_interrupted(self, last_msg_ts, channel, client)` (줄 388): 인터럽트로 중단된 실행의 사고 과정 메시지 정리
- `_handle_success(self, result, channel, thread_ts, last_msg_ts, say, client, is_thread_reply)` (줄 396): 성공 결과 처리
- `_handle_error(self, error, channel, thread_ts, last_msg_ts, say, client, is_thread_reply)` (줄 456): 오류 결과 처리
- `_handle_help(self, say, thread_ts)` (줄 481): help 명령어
- `_handle_status(self, say, thread_ts)` (줄 494): status 명령어
- `_handle_compact(self, say, client, thread_ts, parent_thread_ts)` (줄 505): compact 명령어
- `handle_mention(self, event, say, client)` (줄 534): 멘션 이벤트 핸들러
- `handle_message(self, event, say, client)` (줄 588): 스레드 메시지 핸들러
