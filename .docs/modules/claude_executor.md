...
- `run(self, session, prompt, msg_ts, channel, say, client, role, trello_card, is_existing_thread, initial_msg_ts, dm_channel_id, dm_thread_ts)` (줄 117): 세션 내에서 Claude Code 실행 (공통 로직)
- `_handle_intervention(self, thread_ts, prompt, msg_ts, channel, say, client, role, trello_card, is_existing_thread, initial_msg_ts, dm_channel_id, dm_thread_ts)` (줄 180): 인터벤션 처리: 실행 중인 스레드에 새 메시지가 도착한 경우
...
- `_replace_thinking_message(self, client, channel, old_msg_ts, new_text, new_blocks, thread_ts)` (줄 535): 사고 과정 메시지를 최종 응답으로 교체 (chat_update)
- `_handle_interrupted(self, last_msg_ts, main_msg_ts, is_trello_mode, trello_card, session, channel, client, dm_channel_id, dm_last_reply_ts)` (줄 560): 인터럽트로 중단된 실행의 사고 과정 메시지 정리
