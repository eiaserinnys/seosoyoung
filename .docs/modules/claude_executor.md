...
- `_handle_restart_marker(self, result, session, thread_ts, say)` (줄 803): 재기동 마커 처리
- `_handle_list_run_marker(self, list_name, channel, thread_ts, say, client)` (줄 826): LIST_RUN 마커 처리 - 정주행 시작
- `_handle_error(self, error, is_trello_mode, trello_card, session, channel, last_msg_ts, main_msg_ts, say, client, is_thread_reply)` (줄 895): 오류 결과 처리
- `_handle_exception(self, e, is_trello_mode, trello_card, session, channel, thread_ts, last_msg_ts, main_msg_ts, say, client, is_thread_reply)` (줄 938): 예외 처리
- `_handle_image_gen(self, prompts, channel, thread_ts, say, client)` (줄 978): 이미지 생성 마커 처리
