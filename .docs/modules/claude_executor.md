...
- `run(self, prompt, thread_ts, msg_ts)` (줄 134): 세션 내에서 Claude Code 실행 (공통 로직)
- `_handle_intervention(self, thread_ts, prompt, msg_ts)` (줄 196): 인터벤션 처리: 실행 중인 스레드에 새 메시지가 도착한 경우
- `_run_with_lock(self, thread_ts, prompt, msg_ts)` (줄 237): 락을 보유한 상태에서 실행 (while 루프로 pending 처리)
- `_execute_once(self, thread_ts, prompt, msg_ts)` (줄 290): 단일 Claude 실행
- `_get_role_config(self, role)` (줄 362): 역할에 맞는 runner 설정을 반환 (모듈 함수에 위임)
- `_get_service_adapter(self)` (줄 366): Remote 모드용 ClaudeServiceAdapter를 lazy 초기화하여 반환
- `_register_session_id(self, thread_ts, session_id)` (줄 384): thread_ts ↔ session_id 매핑 등록 및 버퍼된 인터벤션 flush
- `_unregister_session_id(self, thread_ts)` (줄 408): thread_ts ↔ session_id 매핑 해제
- `_get_session_id(self, thread_ts)` (줄 417): thread_ts에 대응하는 session_id 조회
- `_execute_remote(self, thread_ts, prompt)` (줄 422): Remote 모드: soul 서버에 실행을 위임
- `_process_result(self, presentation, result, thread_ts)` (줄 485): 실행 결과 처리
