...
- `_preemptive_compact(self, thread_ts, channel, card_name)` (줄 775): 카드 완료 후 선제적 컨텍스트 컴팩트
- `_start_list_run(self, list_id, list_name, cards)` (줄 822): 리스트 정주행 시작
- `_process_list_run_card(self, session_id, thread_ts, run_channel)` (줄 890): 리스트 정주행 카드 처리
- `_process_list_run_card_inner(self, list_runner, session_id, thread_ts, channel, run_channel)` (줄 932): _process_list_run_card의 실제 로직 (전역 try-except로 감싸기 위해 분리)
