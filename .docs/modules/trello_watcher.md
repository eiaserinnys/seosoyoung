...
- `_start_list_run(self, list_id, list_name, cards)` (줄 798): 리스트 정주행 시작
- `_process_list_run_card(self, session_id, thread_ts, run_channel)` (줄 866): 리스트 정주행 카드 처리
- `_build_list_run_prompt(self, card, session_id, current, total)` (줄 1021): 리스트 정주행용 프롬프트 생성
