...
- `__init__(self, base_dir)` (줄 170): 
- `_meta_path(self, channel_id)` (줄 173): 
- `_read_meta(self, channel_id)` (줄 176): 
- `_write_meta(self, channel_id, meta)` (줄 186): 
- `_prune_history(self, history)` (줄 194): 2시간 초과 항목을 제거합니다.
- `record(self, channel_id, entry_type)` (줄 199): 개입 이력을 기록합니다.
- `minutes_since_last(self, channel_id)` (줄 211): 마지막 개입으로부터 경과 시간(분)을 반환합니다.
- `recent_count(self, channel_id, window_minutes)` (줄 225): 최근 window_minutes 내 개입 횟수를 반환합니다.
- `can_react(self, channel_id)` (줄 236): 이모지 리액션은 항상 허용
- `filter_actions(self, channel_id, actions)` (줄 240): 액션을 필터링합니다.
