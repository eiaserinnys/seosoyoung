...
- `__init__(self)` (줄 68): 
- `new_card(self)` (줄 72): 새 카드 ID 생성 및 현재 카드로 설정
- `current_card_id(self)` (줄 82): 현재 활성 카드 ID (thinking 블록 없이 tool이 오면 None)
- `set_last_tool(self, tool_name)` (줄 86): 마지막 도구 이름 기록 (TOOL_RESULT에서 tool_name 폴백용)
- `last_tool(self)` (줄 91): 마지막으로 호출된 도구 이름
...
- `__init__(self, workspace_dir, pool, rate_limit_tracker, credential_store)` (줄 152): 
- `_resolve_mcp_config_path(self)` (줄 164): WORKSPACE_DIR 기준으로 mcp_config.json 경로를 해석
- `async execute(self, prompt, resume_session_id, get_intervention, on_intervention_sent)` (줄 171): Claude Code 실행 (SSE 이벤트 스트림)
