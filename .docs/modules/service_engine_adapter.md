...
- `__init__(self, workspace_dir)` (줄 99): 
- `_resolve_mcp_config_path(self)` (줄 104): WORKSPACE_DIR 기준으로 mcp_config.json 경로를 해석
- `async execute(self, prompt, resume_session_id, get_intervention, on_intervention_sent)` (줄 111): Claude Code 실행 (SSE 이벤트 스트림)
