...
- [`tools/npc_chat.py`](modules/tools_npc_chat.md): NPC 대화 모듈: 캐릭터 로더, 프롬프트 빌더, npc_list_characters 도구.
...
- `CharacterLoader` (seosoyoung/mcp/tools/npc_chat.py:25): eb_lore 캐릭터 YAML 파일을 로드하고 필드를 추출한다.
- `PromptBuilder` (seosoyoung/mcp/tools/npc_chat.py:121): 캐릭터 데이터를 프롬프트 템플릿에 채워 시스템 프롬프트를 생성한다.
...
- `slack_attach_file()` (seosoyoung/mcp/server.py:18): 슬랙에 파일을 첨부합니다.
- `slack_get_context()` (seosoyoung/mcp/server.py:34): 현재 슬랙 대화의 채널/스레드 정보를 반환합니다.
- `slack_post_message()` (seosoyoung/mcp/server.py:44): 봇 권한으로 슬랙 채널에 메시지를 보냅니다.
- `async slack_generate_image()` (seosoyoung/mcp/server.py:65): 텍스트 프롬프트로 이미지를 생성하고 슬랙 스레드에 업로드합니다.
- `async slack_download_thread_files()` (seosoyoung/mcp/server.py:88): 스레드 내 모든 메시지의 첨부 파일을 다운로드합니다.
- `slack_get_user_profile()` (seosoyoung/mcp/server.py:102): Slack 사용자의 프로필 정보를 조회합니다.
- `async slack_download_user_avatar()` (seosoyoung/mcp/server.py:114): Slack 사용자의 프로필 이미지를 다운로드합니다.
- `npc_list_characters()` (seosoyoung/mcp/server.py:129): 대화 가능한 NPC 캐릭터 목록을 반환합니다.
...
- `npc_list_characters()` (seosoyoung/mcp/tools/npc_chat.py:171): 대화 가능한 NPC 캐릭터 목록을 반환한다.
